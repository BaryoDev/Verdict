...
namespace Upshot.Benchmarks;

/// <summary>
/// Competitive benchmarks comparing Upshot against:
...
public class CompetitiveBenchmarks
{
    ...
    [Benchmark(Description = "Upshot (Success)")]
    public int Upshot_Success()
    {
        var sum = 0;
        for (int i = 0; i < Iterations; i++)
        {
            var result = DivideWithUpshot(100, 2);
            sum += result.Value;
        }
        return sum;
    }
    ...
    [Benchmark(Description = "Upshot (Failure)")]
    public int Upshot_Failure()
    {
        var errorCount = 0;
        for (int i = 0; i < Iterations; i++)
        {
            var result = DivideWithUpshot(100, 0);
            if (result.IsFailure)
            {
                errorCount++;
            }
    ...
    [Benchmark(Description = "Upshot (Mixed)")]
    public int Upshot_Mixed()
    {
        var sum = 0;
        for (int i = 0; i < Iterations; i++)
        {
            var result = DivideWithUpshot(100, i % 10 == 0 ? 0 : 2);
            if (result.IsSuccess)
            {
                sum += result.Value;
            }
    ...
    private static Upshot.Result<int> DivideWithUpshot(int numerator, int denominator)
    {
        if (denominator == 0)
            return Upshot.Result<int>.Failure("DIVIDE_BY_ZERO", "Cannot divide by zero");
        return Upshot.Result<int>.Success(numerator / denominator);
    }
}
